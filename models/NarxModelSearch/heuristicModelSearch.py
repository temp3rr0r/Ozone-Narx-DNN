...
if __name__ == "__main__":
    ...
    print('Loading data...')
    r = np.genfromtxt("data/BETN073.csv", delimiter=',')
    r = np.delete(r, [0], axis=1)  # Remove dates

    r = np.delete(r, [1, 2, 3, 4, 5, 6], axis=1)  # Remove all other variables

    # sensor_mean = pd.read_pickle("hass_ts_mean.pkl")['sensor.flower1_moisture']
    # sensor_std = pd.read_pickle("hass_ts_std.pkl")['sensor.flower1_moisture']
    ...
    if modelLabel == 'rand':
        randomModelSearch(x_data_3d, y_data)
    elif modelLabel == 'pso':
        particleSwarmOptimizationModelSearch(x_data_3d, y_data)
    elif modelLabel == 'de':
        differentialEvolutionModelSearch(x_data_3d, y_data)
    elif modelLabel == 'bh':
        basinHoppingpModelSearch(x_data_3d, y_data)

...
from keras.layers import GaussianNoise, LSTM, BatchNormalization, Bidirectional, Dense
from keras.regularizers import l1_l2
from keras.models import Sequential
from keras.optimizers import adadelta, adagrad, adam, nadam, adamax
...
def train_model(x, *args):
    ...
    # lstm_kwargs['kernel_regularizer'] = l1_l2(recurrent_dropout2, dropout2)  # TODO: mini rand: 50% for (0 - 0.01)
    # if use_gaussian_noise3 > 0.5:
    #     lstm_kwargs['activity_regularizer'] = l1_l2(recurrent_dropout3, dropout3)
    # if use_gaussian_noise2 > 0.5:
    #     lstm_kwargs['bias_regularizer'] = l1_l2(noise_stddev2, noise_stddev3)
    # # if np.random.uniform(0, 1) > 0.5:
    # #     lstm_kwargs['stateful'] = True
    # #     batch_input_shape = (batch_size, timesteps, data_dim)
    # #     lstm_kwargs['batch_input_shape'] = (x_data.shape[1], x_data.shape[2])
    # # model.add(.CuDNNLSTM(**lstm_kwargs, )  # TODO: test speed
    # # model.add(Bidirectional(.CuDNNLSTM(**lstm_kwargs)))  # TODO: test speed
    # model.add(Bidirectional(LSTM(**lstm_kwargs)))  # TODO: test speed
    ...
    #     model.add(.AlphaDropout(np.random.uniform(0.001, 0.1)))
    # if useBatchNormalization3 > 0.5:
    #     model.add(.GaussianDropout(np.random.uniform(0.001, 0.1)))
    # if use_gaussian_noise1 > 0.5:
    #     model.add(GaussianNoise(noise_stddev1))
    # if useBatchNormalization1 > 0.5:
    #     model.add(BatchNormalization())
    # model.add(Dense(units2, activation=random.choice(
    ...
    if random.random() > 0.5:
        lstm_kwargs['activity_regularizer'] = l1_l2(random.uniform(0.0, 0.02),
                                                                          random.uniform(0.0, 0.02))
    if random.random() > 0.5:
        lstm_kwargs['bias_regularizer'] = l1_l2(random.uniform(0.0, 0.02),
                                                                      random.uniform(0.0, 0.02))
    if random.random() > 0.5:
        lstm_kwargs['kernel_regularizer'] = l1_l2(random.uniform(0.0, 0.02),
                                                                        random.uniform(0.0, 0.02))
    model.add(Bidirectional(LSTM(**lstm_kwargs)))
    if use_gaussian_noise2 > 0.5:
    ...
    if optimizer == 'amsgrad':  # TODO: test amsgrad, special version of adam
        model.compile(loss='mean_squared_error', optimizer=adam(amsgrad=True))
    else:
        model.compile(loss='mean_squared_error', optimizer=optimizer)
    ...
    plot_model(model, show_shapes=True, to_file='foundModels/{}Iter{}Rank{}Model.png'.format(modelLabel,
                                                                                                            train_model.counter, rank))

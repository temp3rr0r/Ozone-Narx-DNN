...
debug = True
def convert_1D_to_nD_index(rank, size, cellular_automata_dimensions):
    returning_nD_index = []
...
def convert_nD_to_1D_index(index_nD, size, cellular_automata_dimensions):
...
def get_1D_neighbours(rank, size, cellular_automata_dimensions):
    returning_1D_neighbours = []
    index_nD = convert_1D_to_nD_index(rank, size, cellular_automata_dimensions)
    for idx in range(len(cellular_automata_dimensions)):
        ...
        if new_nD[idx] >= cellular_automata_dimensions[idx]:
            ...
            new_1D = convert_nD_to_1D_index(new_nD, size, cellular_automata_dimensions)
            if new_1D >= size:
                new_nD[idx] = 0
        new_1D = convert_nD_to_1D_index(new_nD, size, cellular_automata_dimensions)
...
def get_cellular_automata_linear_selection_neighbour_1D_index(rank, size, cellular_automata_dimensions, grid_mse_rank_list):

    if len(cellular_automata_dimensions) > 1:
        oneD_neighbours = get_1D_neighbours(rank, size, cellular_automata_dimensions)
        if debug:
            print("oneD_neighbours: {}".format(oneD_neighbours))
...
if __name__ == '__main__':
    ...
    CA_dimensions = [5, 4]
    lr_selected_rank = get_cellular_automata_linear_selection_neighbour_1D_index(current_mpi_rank, mpi_size, CA_dimensions, agentsBuffer_mse)
